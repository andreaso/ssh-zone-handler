...
sed -e "s/__PRIMARY_IP__/${primary_ip}/" < /mp/devel/named.conf.secondary.in \
    | install --owner=root --group=root --mode=0644 --no-target-directory /dev/stdin /etc/bind/named.conf.local
...
systemctl restart named.service ssh.service
